...
def get_ai_preferences(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT user_id, openai_api_key, claude_api_key, gemini_api_key, preferred_provider, created_at, updated_at
            FROM ai_user_preferences WHERE user_id = ?
        ''', (user_id,))
        result = cursor.fetchone()
        if result:
            # Decrypt API keys if they exist
            decrypted_openai = decrypt_data(result[1]) if result[1] else None
            decrypted_claude = decrypt_data(result[2]) if result[2] else None
            decrypted_gemini = decrypt_data(result[3]) if result[3] else None
            
            return {
                "user_id": result[0],
                "openai_api_key": decrypted_openai,
                "claude_api_key": decrypted_claude,
                "gemini_api_key": decrypted_gemini,
...
def get_ai_client_for_user(user_id: str) -> Optional[Dict[str, Any]]:
    """
    Get AI client configuration for a user (for AutoBot integration)
    Args:
        user_id: User identifier
    Returns:
        Dict with AI client configuration or None if not found
    """
    try:
        ...
        if preferred == "openai" and preferences.get("openai_api_key"):
            return {
                "provider": "openai",
                "api_key": preferences["openai_api_key"],
                "user_id": user_id
            }
        elif preferred == "claude" and preferences.get("claude_api_key"):
            return {
                "provider": "claude", 
                "api_key": preferences["claude_api_key"],
                "user_id": user_id
            }
        elif preferred == "gemini" and preferences.get("gemini_api_key"):
            return {
                "provider": "gemini",
            ...
            elif preferences.get("claude_api_key"):
                return {
                    "provider": "claude",
                    "api_key": preferences["claude_api_key"],
                    "user_id": user_id
                }
            elif preferences.get("gemini_api_key"):
                return {
                    "provider": "gemini",
                    "api_key": preferences["gemini_api_key"],
                    "user_id": user_id
        ...
        logger.error(f"Error getting AI client for user {user_id}: {str(e)}")
        return None
